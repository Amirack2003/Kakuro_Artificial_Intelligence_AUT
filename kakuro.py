...
from Mcv import mcv_solve
from backtrack import simple_solve
...
class KakuroUI(Frame):
    ...
    def key_pressed(self, event):
        ...
        if self.game.game_over:
            return
        if self.row >= 0 and self.col >= 0 and event.char in "123456789" and event.char != '' and [self.row,
                                                                                                   self.col] in self.game.data_fills:
            found_flag = False
        ...
        for line in file:
            ...
            if readstatus == 1 and line.rstrip("\r\n").isdigit():
                ...
                if line[0] == 'e':
                    ...
                    self.game.data_totals = self.game.data_totals + [
